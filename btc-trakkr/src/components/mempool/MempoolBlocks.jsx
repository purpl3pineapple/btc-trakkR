...
const MempoolBlocks = () => {
  ...
  const { current, loading } = useSelector((state) => state.mempool.blocks);
  ...
  return (
    <MempoolDataSection
      id="mempool-blocks"
      title="Mempool Past Block Info"
      links={[
        {
          title: "Transactions",
          to: "#mempool-recent-txs",
        },
      ]}
      content={
        loading ? (
              ...
              {current.map((block, idx) => (
                <tr key={idx}>
                  ...
                  <td>
                    ...
                    {block.extras === undefined
                      ? "***"
                      : Number(block.extras.medianFee).toFixed()}{" "}
                    sat/vB
                  </td>
                  <td>
                    {block.extras === undefined
                  ...
                  <td className="text-nowrap">
                    {Number(block.size / 1000000).toFixed(1)} MB
                  </td>
                  ...
                  <td>
                    {new Date(block.mediantime * 1000).toLocaleString("en-US")}
                  </td>
                  <td>{block.nonce}</td>
                </tr>
              ))}
